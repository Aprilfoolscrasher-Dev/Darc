...
local function createSpeedTag(character)
    ...
    if head:FindFirstChild("SpeedTag") then
        head.SpeedTag:Destroy()
    end

    -- Billboard GUI (SMALLER)
    ...
    label.TextStrokeTransparency = 0.2 -- softer outline
    ...
    RunService.RenderStepped:Connect(function()
        if character and hrp then
            ...
            label.Text = "SPD: " .. math.floor(speed)
        end
    end)
end
...
if player.Character then
    createSpeedTag(player.Character)
end
...
-- ============================================================
-- CONFIG SYSTEM (IMPROVED)
-- ============================================================

local CONFIG_FILE = "sinx_duels_config.json"
...
--// SINX FINAL: AUTO MEDUSA + HIT CIRCLE (TRUE FLOAT SAFE)
...
local function startSystem()
    ...
    -- 💜 VISUAL HIT CIRCLE (NO PHYSICS)
    Cebo.Circle = Instance.new("Part")
    Cebo.Circle.Shape = Enum.PartType.Cylinder
    Cebo.Circle.Material = Enum.Material.Neon
    Cebo.Circle.Size = Vector3.new(0.05, MEDUSA_RANGE * 2, MEDUSA_RANGE * 2)
    Cebo.Circle.Color = Color3.fromRGB(170, 85, 255)
    Cebo.Circle.Transparency = 0.15
    Cebo.Circle.CanCollide = false
    Cebo.Circle.Anchored = true -- 🔥 KEY FIX
    Cebo.Circle.Parent = workspace

    -- LOOP
    Cebo.Conn = RunService.RenderStepped:Connect(function()
        if not Enabled.AutoMedusa then return end
        if not char or not hrp then return end

        -- 💜 FOLLOW PLAYER (NO WELD)
        Cebo.Circle.CFrame =
            CFrame.new(hrp.Position - Vector3.new(0, 1, 0)) *
            CFrame.Angles(0, 0, math.rad(90))

        local target = nil
        local dmin = MEDUSA_RANGE
        ...
        if target then
            -- 🐍 AUTO MEDUSA ONLY (NO ROTATION / NO PHYSICS TOUCH)
            local medusa = getMedusa()
            if medusa then
                if medusa.Parent ~= char then
                    medusa.Parent = char
                end

                if tick() - lastUse >= SPAM_DELAY then
                    medusa:Activate()
                    lastUse = tick()
                end
            end
...
local medusaPart
local lastUse = 0
local AutoMedusaEnabled = true
local MedusaInitialized = true
...
local function startFloat()
    ...
    local targetY = hrp.Position.Y + FLOAT_HEIGHT
    ...
    floatConn = RunService.Heartbeat:Connect(function()
        ...
        if descending then
            vertVel = -15
            if currentY <= targetY - FLOAT_HEIGHT + 0.5 then
            ...
            -- 💜 SMOOTH FLOAT CONTROL (NO SPIKES)
            vertVel = math.clamp(diff * 4, -20, 20)
        ...
        if moveDir.Magnitude > 0.1 then
            horiz = moveDir.Unit * Values.BoostSpeed
        end

        -- 💜 FINAL VELOCITY (SMOOTH BLEND)
...
local buttonData = {
    ...
    --{name = "auto [med]", pos = UDim2.new(0.7, 0, 0.42, 0), func = "automed"},
            ...
            while tauntActive do
                ...
                local success = pcall(function()
                    ...
                    if generalChannel then
                        generalChannel:SendAsync("/sinxs hub")
                    end
                end)
                ...
                if not success then
                    pcall(function()
                        ...
                        if SayMessageRequest then
                            ...
                            if SayMessageRequest then
                                SayMessageRequest:FireServer("/sinxs hub", "All")
                            end
                        end
                    end)
                end
...
-- 🔥 SINX COLORS
button.BackgroundColor3 = Color3.fromRGB(35, 10, 60) -- deep purple
...
stroke.Thickness = 2
stroke.Color = Color3.fromRGB(180, 0, 255) -- neon purple
...
arrow.TextSize = 20
arrow.TextColor3 = Color3.fromRGB(200,120,255) -- soft purple glow
